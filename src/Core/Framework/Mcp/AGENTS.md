...
Every registered tool belongs to a group. The `default` group holds the always-advertised meta-tools and is never an enable-able toolset. Core and plugin tools declare their group with `#[McpToolGroup]` at compile time; app tools (loaded at runtime, so they carry no attribute) are grouped under their owning app's technical name via `AppMcpPrivilegeProvider::getAppToolGroups()`, so each app forms its own toolset. Anything still without a group falls to the `other` catch-all, which is itself an enable-able toolset so that no allowlisted tool is ever reachable through `shopware-tool-search` alone.
