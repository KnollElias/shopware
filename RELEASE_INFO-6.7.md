...
- `composer admin:setup-extension-tooling` discovers all installed extensions with Administration sources (warning with the `bundle:dump` command when `var/plugins.json` is stale) and generates disposable, marker-owned configs (per-extension tsconfigs under `var/admin-extension-tooling/`, root `tsconfig.json` / `eslint.config.mjs` projections, IDE bootstraps for VS Code and Zed, a managed ignore block in the project `.gitignore`). A zero-config plugin gets full typed autocomplete — including installation-specific entity types — and lint coverage without any files of its own. A plugin is included with one command, `--shim=<name>`, which writes a self-ignoring `.shopware-admin/` bridge (holding the machine-specific paths, merging plugin aliases from a committed `tsconfig.aliases.json`) and, for a plugin with no config yet, two small committable `tsconfig.json` / `eslint.config.mjs` that just extend it — visible, committable, and freely customizable as long as the `extends` stays; existing configs are never overwritten. The report classifies extensions (ready / bridged / bridge-unwired / custom), lists changed files by path, and prints only the actually-missing next step.
- `composer admin:check-extensions` runs `vue-tsc` and ESLint per extension with the Administration's pinned tool versions and readable, colorized output. In an interactive terminal it opens a picker to select which extensions to check; `--only=<name[,name]>` or `--all` skip the prompt (non-interactive shells check all). Extension configs that do not compose the Shopware preset are visibly skipped with a reason-specific `why:` and the exact `fix:`, never silently green; TypeScript runs are blocked with the fix command while the entity schema is missing; JavaScript-only extensions report a qualified pass instead of a vacuous green; findings in `vendor/`-installed extensions are reported non-fatally by default. `--fix` applies ESLint autofixes, `--show-commands` prints the underlying tool invocations.

Both commands validate their options strictly: `-- --help` prints a generated reference and unknown flags abort with exit 2 before anything is written (note Composer swallows options placed before the `--` separator).
